...
@cachetools.func.ttl_cache(ttl=600)
def load_world(baseURL=None, confirmedURL=None, deathURL=None):

    sources = {
    'confirmed' : confirmedURL or ((baseURL or 'https://github.com/CSSEGISandData/COVID-19/raw/master/csse_covid_19_data/csse_covid_19_time_series/') + 'time_series_covid19_confirmed_global.csv'),
    'death' : deathURL or ((baseURL or 'https://github.com/CSSEGISandData/COVID-19/raw/master/csse_covid_19_data/csse_covid_19_time_series/') + 'time_series_covid19_deaths_global.csv')
}
...
@cachetools.func.ttl_cache(ttl=600)
def get_fips_codes(url=None):
    '''Get valid FIPS codes from covid19forecasthub'''
    url = url or 'https://raw.githubusercontent.com/reichlab/covid19-forecast-hub/master/data-locations/locations.csv'
    df = pd.read_csv(url)
...
@cachetools.func.ttl_cache(ttl=600)
def get_county_info(url=None, fipsURL=None):
    '''Get state info from JHU location lookup file'''
    
    url = url or 'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/UID_ISO_FIPS_LookUp_Table.csv'
    df = pd.read_csv(url)
    
    df = filter_counties(df, fipsURL)
...
@cachetools.func.ttl_cache(ttl=600)
def load_us(counties=False, baseURL=None, fipsURL=None, confirmedCountryURL=None, deathCountryURL=None):
    
    baseURL = baseURL or "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/"

    def load_us_time_series(url):
    ...
    confirmed = load_us_time_series(confirmedCountryURL or (baseURL + "time_series_covid19_confirmed_US.csv"))
    deaths = load_us_time_series(deathCountryURL or (baseURL + "time_series_covid19_deaths_US.csv"))
